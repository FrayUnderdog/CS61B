public class LinkedListDeque<T> {
    ...
    public void addLast(T item) {
        // if LLD is empty, oldLast will be null, so need to modify
        if (size == 0) {
            Node newLast = new Node(item, sentinal, sentinal);  // oldLast <- newLast
        } else {
            Node oldLast = getLast();
            // oldLast <- newLast
            oldLast.next = new Node(item, oldLast, sentinal); // oldLast -> newLast
        }
        size ++;
    }
